  ...
  const generateCertificate = async (activity) => {
    ...
    try {
      ...
      setCertificateData({
        ...
        eventLocation: activity.activityDetails?.location || translations.location || "Location",
  ...
  const renderActivityItem = (
    ...
    >
      ...
      {showQR ? (
        ...
        <div className="flex flex-col gap-2 items-end justify-center min-w-[160px]">
          {activity.activityDetails?.feedbackFormStatus === "accepting" && (
            <Button
              variant="outline"
              size="sm"
              className="border-[#1CAC78] text-[#1CAC78] hover:bg-[#1CAC78] hover:text-white"
              onClick={() => router.push(`/feedback/${activity.activityId}`)}
            >
              Fill Feedback
            </Button>
          )}
