  ...
  useEffect(() => {
    const fetchProductsAndRes = async () => {
      try {
        ...
        // Fetch products specifically for this NGO
        const productsQuery = query(collection(db, "products"), where("ngoId", "==", ngoId));
        ...
        // Fetch res collection specifically for this NGO
        const resQuery = query(collection(db, "res"), where("ngoId", "==", ngoId));
        const resSnapshot = await getDocs(resQuery);
        const resItems = resSnapshot.docs.map(doc => {
          const data = doc.data();
          // Calculate actual available quantity for donations (fallback to initial quantity if remaining isn't set yet)
          const quantity = data.remaining !== undefined ? data.remaining : (data.quantity || 0);
          return {
            id: doc.id,
            type: 'res',
