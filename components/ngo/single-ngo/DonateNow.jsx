...
const DonateNow = ({ ngoData, ngoId }) => {
  ...
  const resolvedNgoId = ngoData?.ngoId || ngoId;
  const razorpayKeyId = ngoData?.donationsData?.razorpayKeyId;
  const razorpayKeySecret = ngoData?.donationsData?.razorpayKeySecret;
  ...
  const handlePayment = useCallback(async () => {
    try {
      ...
      if (!resolvedNgoId) {
        toast.error("Unable to identify NGO for donation.");
        return;
      }

      if (!razorpayKeyId || !razorpayKeySecret) {
        toast.error("Online donation is not configured for this NGO.");
        return;
      }
      ...
      const response = await fetch("/api/create-donation", {
        ...
        body: JSON.stringify({
          ...
          rzpKeySecret: razorpayKeySecret,
        }),
      });
      const responseData = await response.json();

      if (!response.ok) {
        throw new Error(responseData?.error || "Unable to create payment order");
      }

      const { orderId, amount } = responseData;
  ...
  }, [onlineFormData, resolvedNgoId, razorpayKeyId, razorpayKeySecret]);
  ...
  const saveDonationData = useCallback(
    async (paymentResponse) => {
      try {
        ...
        const donationData = {
          ...
          donatedOn: donationTimestamp.split("T")[0],
        ...
        try {
          ...
          console.error("Error sending donation SMS:", smsError);
        }
        ...
        const ngoDocRef = doc(db, "ngo", resolvedNgoId);
    ...
    [onlineFormData, ngoData, resolvedNgoId]
  );
