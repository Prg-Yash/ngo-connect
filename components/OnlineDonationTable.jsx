      ...
      (snapshot) => {
        ...
        const formattedDonations = allCompletedDonations.map((donation) => {
          ...
          return {
            ...
            transactionId:
              donation.transactionId || donation.paymentId || donation.orderId || "N/A",
  ...
  const handleDonationApprovalSnapshot = (snapshot, ngoId) => {
    ...
    snapshot.forEach((doc) => {
      ...
      const formattedDonation = {
        ...
        transactionId: data.transactionId || data.paymentId || data.orderId || "N/A",
  ...
  const extractDonationDate = (donation) => {
    ...
    if (donation.donatedOn) {
      ...
      if (typeof donation.date === "string") {
        return donation.date.includes("T")
          ? donation.date.split("T")[0]
          : donation.date;
      }

      if (donation.date instanceof Date) {
        return donation.date.toISOString().split("T")[0];
      }
