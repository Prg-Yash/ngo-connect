...
import { ENABLE_CRYPTO_DONATIONS } from "@/constants";
import {
  Select,
  SelectContent,
  SelectItem,
...
export function DonorsTable() {
  ...
  const setupRealtimeListeners = async (userDataFromFirestore) => {
    ...
    try {
      ...
      const donationTypes = ENABLE_CRYPTO_DONATIONS
        ? ["cash", "crypto", "resources", "online"]
        : ["cash", "resources", "online"];
  ...
  const updateDonationsState = (donationsData) => {
    ...
    const formattedDonations = donationsData.map((donation) => {
      const parsedDate = parseDonationDate(donation);
      ...
      return {
        ...
        date:
          donation.donationType === "Online" && parsedDate
  ...
  const donationTypeCount = {
    ...
    crypto: ENABLE_CRYPTO_DONATIONS
      ? allDonations.filter((d) => d.type === "Crypto").length
      : 0,
    ...
    switch (transaction.type) {
      ...
      case "Online":
        const resolvedTransactionId =
          data.transactionId || data.paymentId || data.orderId;
          ...
          <>
            ...
            {resolvedTransactionId && (
              <div>
                ...
                <p className="mt-1">{resolvedTransactionId}</p>
              </div>
            )}
            {data.timestamp && (
              <div>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          View all donation records here (cash, resources, and online)
